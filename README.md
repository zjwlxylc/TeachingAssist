...
本仓库当前已完成“阶段 1：项目基础框架搭建”“阶段 2：系统管理与部署基础”“阶段 3：课程、班级、课堂与学生导入”“阶段 4：课堂状态机与签到系统”和“阶段 5：课堂公告与 WebSocket 实时通信”，提供可运行的 FastAPI 后端、React/Vite 前端、SQLite 初始化与迁移、教师认证、网卡端口检测、防火墙引导、数据库备份恢复基础能力、课前准备、学生名单导入、课堂开始/结束、学生签到、课堂公告和实时推送能力。
...
- 数据库：SQLite + WAL 模式
- 实时通信：WebSocket 已接入课堂公告推送，后续问答/统计可复用同一通道
