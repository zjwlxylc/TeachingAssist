...
本仓库当前已完成“阶段 1：项目基础框架搭建”“阶段 2：系统管理与部署基础”“阶段 3：课程、班级、课堂与学生导入”“阶段 4：课堂状态机与签到系统”“阶段 5：课堂公告与 WebSocket 实时通信”“阶段 6：课堂互动问答 P0”“阶段 7：作业管理 P0”“阶段 8：AI 管理、降级与内容安全”“阶段 9：P1 增强功能开发”和“阶段 10：打包、部署与试点”，提供可运行的 FastAPI 后端、React/Vite 前端、SQLite 初始化与迁移、教师认证、网卡端口检测、防火墙引导、数据库备份恢复基础能力、课前准备、学生名单导入、课堂开始/结束、学生签到、课堂公告、实时推送、课堂问答发布、学生答题、自动判分、教师统计、作业发布、学生提交、教师查看作业提交列表、AI Provider 配置、自检、降级任务、内容安全检查、P1 导入/签到/问答/作业增强、学习效果评估、断线恢复、Windows 打包脚本、U 盘部署目录和试点文档。
...
- AI Provider 配置、启用、切换和 API Key 后端保存：已完成
- 启动时 AI 连通性自检和教师端手动自检：已完成
- AI 不可用时基础模式提示和受影响功能展示：已完成
- Excel 映射、简答反馈、作业批阅、学习建议降级策略入口：已完成
- AI 内容安全最大长度、关键词替换/拦截、展示策略配置：已完成
- AI 自检日志、内容安全日志和失败任务记录：已完成

阶段说明见 `docs/phase-8-delivery.md`。

## 阶段 9 交付状态

- AI/本地字段映射建议、错误报告导出、增量导入和学生停启用：已完成
- 签到数据导出、手动补签、签到状态修改和修改日志：已完成
- 简答题 AI 反馈、问答加分、答案导出、匿名统计和草稿恢复：已完成
- 作业教师附件、AI 批阅、教师复核、成绩发布、学生反馈和导出：已完成
- 单堂课学习效果临时/最终评估、加权分级、异常预警和 CSV 导出：已完成
- 学生端断线请求缓存重发、中断事件记录、延长答题和重开签到：已完成

阶段说明见 `docs/phase-9-delivery.md`。

## 阶段 10 交付状态

- 前端静态资源构建和后端静态托管：已完成
- Windows PyInstaller 打包脚本：已完成
- 启动器脚本：已完成
- U 盘目录生成脚本和默认本地配置：已完成
- 教师使用手册：已完成
- 部署检查清单：已完成
- 故障排查手册：已完成
- 试点反馈报告模板：已完成

阶段说明见 `docs/phase-10-delivery.md`。
