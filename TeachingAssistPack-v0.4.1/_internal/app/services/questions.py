...
def get_student_answer_summary(
    ...
    with get_connection() as connection:
        ...
        rows = connection.execute(
            ...
            SELECT q.id, q.title, q.question_type, q.score,
                   ...
                   qa.quality_score, qa.ai_feedback_status, qa.ai_feedback_json
            FROM questions q
    ...
    for row in rows:
        ...
        raw_feedback = item.get("ai_feedback_json")
        try:
            ai_feedback = json.loads(raw_feedback) if raw_feedback else None
        ...
        answers.append(
            {
                ...
                "ai_feedback_status": item.get("ai_feedback_status"),
...
def get_question_answers_detail(question_id: int) -> dict[str, Any]:
    ...
    for row in rows:
        item = _row_to_dict(row)
        # 解析 AI 反馈 JSON → 对象，前端直接渲染不用再 parse
        fb_raw = item.get("ai_feedback_json")
        if isinstance(fb_raw, str) and fb_raw.strip():
            try:
                item["ai_feedback"] = json.loads(fb_raw)
            except (json.JSONDecodeError, TypeError):
                item["ai_feedback"] = None
        else:
            item["ai_feedback"] = None
        # 去掉原始 JSON 字符串，前端不需要
        item.pop("ai_feedback_json", None)
        items.append(item)
...
def set_answer_quality_score(answer_id: int, quality_score: int, teacher_id: int | None = None) -> dict[str, Any]:
    """教师手动设置某条答案的质量分（0–3），用于 AI 不可用时手动批阅简答题。"""
    quality_score = max(0, min(3, int(quality_score)))
    with get_connection() as connection:
        answer = connection.execute(
            "SELECT * FROM question_answers WHERE id = ?", (answer_id,)
        ).fetchone()
        if not answer:
            raise AppError(f"答案记录不存在：{answer_id}", status_code=404)
        connection.execute(
            "UPDATE question_answers SET quality_score = ?, ai_feedback_status = 'completed', updated_at = CURRENT_TIMESTAMP WHERE id = ?",
            (quality_score, answer_id),
        )
        return {
            "answer_id": answer_id,
